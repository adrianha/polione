...
const schema = z.object({
  ...
  MARKET_POLL_MS: z.coerce.number().int().positive().default(5000),
  MARKET_URGENT_POLL_MS: z.coerce.number().int().positive().default(1000),
  REDEEM_POLL_MS: z.coerce.number().int().positive().default(60000),
  TELEGRAM_POLL_MS: z.coerce.number().int().positive().default(5000),
...
export const loadConfig = (): BotConfig => {
  ...
  return {
    ...
    marketUrgentPollMs: parsed.MARKET_URGENT_POLL_MS,
    redeemPollMs: parsed.REDEEM_POLL_MS,
    telegramPollMs: parsed.TELEGRAM_POLL_MS,
