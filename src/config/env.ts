...
const schema = z.object({
  ...
  SCHEDULER_TICK_SECONDS: z.coerce.number().positive().default(1),
  DISCOVERY_INTERVAL_SECONDS: z.coerce.number().int().positive().optional(),
  CURRENT_MARKET_INTERVAL_SECONDS: z.coerce.number().int().positive().optional(),
  ENTRY_INTERVAL_SECONDS: z.coerce.number().int().positive().optional(),
  REDEEM_INTERVAL_SECONDS: z.coerce.number().int().positive().optional(),
  TELEGRAM_POLL_INTERVAL_SECONDS: z.coerce.number().int().positive().optional(),
  ...
  V3_MARKET_SLUG_PREFIX: z.string().default("sol-updown-5m"),
  V3_MARKET_INTERVAL_SECONDS: z.coerce.number().int().positive().default(300),
  V3_ENTRY_THRESHOLD: z.coerce.number().min(0).max(1).default(0.85),
  V3_TAKE_PROFIT_PRICE: z.coerce.number().min(0).max(1).default(0.95),
  V3_STOP_LOSS_PRICE: z.coerce.number().min(0).max(1).default(0.75),
  V3_ORDER_SIZE: z.coerce.number().positive().default(5),
  V3_MAX_ENTRY_ASK: z.coerce.number().min(0).max(1).default(0.9),
  V3_MAX_LIVE_POSITIONS: z.coerce.number().int().min(1).default(1),
  V3_LOOP_INTERVAL_SECONDS: z.coerce.number().positive().default(2),
  V3_ORDER_FILL_TIMEOUT_MS: z.coerce.number().int().positive().default(10000),
  V3_ORDER_FILL_POLL_INTERVAL_MS: z.coerce.number().int().positive().default(1000),
  V3_STATE_FILE_PATH: z.string().default(".bot-v3-state.json"),
...
export const loadConfig = (): BotConfig => {
  ...
  return {
    ...
    discoveryIntervalSeconds: parsed.DISCOVERY_INTERVAL_SECONDS,
    currentMarketIntervalSeconds: parsed.CURRENT_MARKET_INTERVAL_SECONDS,
    entryIntervalSeconds: parsed.ENTRY_INTERVAL_SECONDS,
    redeemIntervalSeconds: parsed.REDEEM_INTERVAL_SECONDS,
    telegramPollIntervalSeconds: parsed.TELEGRAM_POLL_INTERVAL_SECONDS,
    ...
    v3MarketSlugPrefix: parsed.V3_MARKET_SLUG_PREFIX,
    v3MarketIntervalSeconds: parsed.V3_MARKET_INTERVAL_SECONDS,
    v3EntryThreshold: parsed.V3_ENTRY_THRESHOLD,
    v3TakeProfitPrice: parsed.V3_TAKE_PROFIT_PRICE,
    v3StopLossPrice: parsed.V3_STOP_LOSS_PRICE,
    v3OrderSize: parsed.V3_ORDER_SIZE,
    v3MaxEntryAsk: parsed.V3_MAX_ENTRY_ASK,
    v3MaxLivePositions: parsed.V3_MAX_LIVE_POSITIONS,
    v3LoopIntervalSeconds: parsed.V3_LOOP_INTERVAL_SECONDS,
    v3OrderFillTimeoutMs: parsed.V3_ORDER_FILL_TIMEOUT_MS,
    v3OrderFillPollIntervalMs: parsed.V3_ORDER_FILL_POLL_INTERVAL_MS,
    v3StateFilePath: parsed.V3_STATE_FILE_PATH,
